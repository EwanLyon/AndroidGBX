...
import { RNSerialport, definitions, actions, ReturnedDataTypes, IDevice } from 'react-native-serialport';
import { GbxDevice } from '../gbxcart/hw_GBxCartRW';
type Props = {};

interface AAAADevice extends IDevice {
	placeholder?: boolean;
}
type State = {
	...
	deviceList: AAAADevice[];
};
class ManualConnection extends Component<Props, State> {
	constructor(props: Props) {
		...
		this.state = {
			...
			deviceList: [{ name: 'Device Not Found', placeholder: true, vendorId: 0, productId: 0 }],
			sendText: '3D',
			returnedDataType: definitions.RETURNED_DATA_TYPES.HEXSTRING as ReturnedDataTypes,
			gbxDevice: new GbxDevice(),
		};
	...
	onServiceStarted(response: { deviceAttached: any }) {
	...
	fillDeviceList() {
		RNSerialport.getDeviceList().then((response) => {
			if (response) {
				...
				Alert.alert('Error from getDeviceList()', 'idk lol');
			}
		});
	}
	devicePickerItems() {
		return this.state.deviceList.map((device, index) =>
			!device.placeholder ? (
				<Picker.Item color="#FFFFFF" key={index} label={device.name} value={device.name} />
			) : (
				<Picker.Item color="#FFFFFF" key={index} label={device.name} value={''} />
			)
		);
	}
	...
	checkSupport() {
		// if (this.state.selectedDevice === undefined || this.state.selectedDevice === '') return;
		RNSerialport.isSupported("/dev/bus/usb/002/002")
			.then((status) => {
	...
	handleConnectButton = async () => {
		...
		if (isOpen) {
			...
			// RNSerialport.connectDevice(this.state.selectedDevice, parseInt(this.state.baudRate, 10));
			RNSerialport.connectDevice("/dev/bus/usb/002/002", parseInt(this.state.baudRate, 10));
		}
	};

	buttonStyle = (status: boolean) => {
			...
			<ScrollView style={styles.body}>
				<View style={styles.container}>
					...
					<View style={styles.inputContainer}>
						...
						<Picker
							enabled={this.state.deviceList.length > 0 && !this.state.deviceList[0].placeholder}
							selectedValue={this.state.selectedDevice}
							onValueChange={(value, index) => {
								console.log(value.toString());
								this.setState({ selectedDevice: value.toString() });
							}}
						>
					...
					<TouchableOpacity
						style={this.buttonStyle(Boolean(this.state.selectedDevice))}
